// Web service to return a directory listing using the => syntax
...
function handle_incoming_request(req, res) {
    ...
    load_album_list((err, albums) => {      // Using local function
        if (err) {
            // Reurn the error to the web service
            // res.writeHead(serverStatus, text);												 
            res.writeHead(500, {"Content-Type": "application/json"});   // 500 internal server 
